...
orange = cv2.imread('2ball800.jpg').astype(np.float64)
apple = cv2.imread('3ball800.png').astype(np.float64)
...
if __name__ == '__main__':
    ...
    directimg=np.hstack((apple[:,:400],orange[:,400:]));
    # directimg = np.hstack((apple[:, :250], orange[:, 250:]));
    ...
    cv2.imshow('Directed Blending images', directimg.astype(np.uint8))
    cv2.imshow('Pyramid Blending images', out.astype(np.uint8))
    cv2.waitKey(0)
